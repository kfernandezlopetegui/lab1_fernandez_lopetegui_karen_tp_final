...
class Auxiliar:
            ...
            for columna in range(0, columnas, step):
                ...
                if (scale != 1):
                    surface_fotograma = pygame.transform.scale(
                        surface_fotograma, (fotograma_ancho_scaled, fotograma_alto_scaled)).convert_alpha()
                if (flip):
                    surface_fotograma = pygame.transform.flip(
                        surface_fotograma, True, False).convert_alpha()
    ...
    def getSurfaceFromSeparateFiles(path_format, from_index, quantity, flip=False, step=1, scale=1, w=0, h=0, repeat_frame=1):
        ...
        for i in range(from_index, quantity+from_index):
            ...
            fotograma_ancho_scaled = int(
                surface_fotograma.get_rect().w * scale)
            fotograma_alto_scaled = int(surface_fotograma.get_rect().h * scale)
            if (scale == 1 and w != 0 and h != 0):
                surface_fotograma = pygame.transform.scale(
                    surface_fotograma, (w, h)).convert_alpha()
            if (scale != 1):
                surface_fotograma = pygame.transform.scale(
                    surface_fotograma, (fotograma_ancho_scaled, fotograma_alto_scaled)).convert_alpha()
            if (flip):
    ...
    @staticmethod
    def escalar_imagen(nombre_imagen: str, escala):
        imagen = pygame.image.load(nombre_imagen)
        fotograma_ancho_scaled = int(
                imagen.get_rect().w * escala)
        fotograma_alto_scaled = int(imagen.get_rect().h * escala)
        if (escala == 1 and fotograma_ancho_scaled != 0 and fotograma_alto_scaled != 0):
